define(function(require, exports, module) {
	...
	exports.init = function() {
		...
		myGrid.setHeader("序号,机器编号,机器标签,账号数,阳光总数");
	...
	function initData(){
        ...
        var req = {jsonContent:'{"function":"F100011","user":{"id":"1","session":"123"},"content":{type:"sun"}}'};
        $.ajax({
            type: "POST",
	...
	function doOnRowDblClicked(rowId){
        //alert(rowId);
        var id= "tab_account_sun_"+rowId;
        var name = "账号-阳光-"+rowId.substring(0,6);
        if(parent._myTabbar.tabs(id)){
            parent._myTabbar.tabs(id).setActive();
        }else{
            parent._myTabbar.addTab(id, name, "180px",null,true);
            parent._myTabbar.tabs(id).attachURL("html/accout/sun/details.html?id="+rowId,null);
        }
	}
});
