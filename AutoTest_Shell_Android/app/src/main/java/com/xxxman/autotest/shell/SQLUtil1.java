...
public class SQLUtil1 {
    ...
    String TAG = SQLUtil1.class.getName();
    ...
    public int selectUserCount(){
        ...
        String sql = "select count(1) as sum from hongbao where day = '"+dateString+"'";
        int sum = 0;
        Cursor c = db.rawQuery(sql, null);
        while (c.moveToNext()) {
    ...
    public int selectHongbaoTaskCount(){
        String sql = "select count(1) as sum from hongbao where hongbao_task_count=0 and day = '"+dateString+"'";
        int sum = 0;
        Cursor c = db.rawQuery(sql, null);
        while (c.moveToNext()) {
            sum = c.getInt(c.getColumnIndex("sum"));
    ...
    public List<User> selectHongbaoUser(){
        String sql = "select id,phone,pwd,number,hongbao  from hongbao where  hongbao_task_count =0 and day = '"+dateString+"'";
        List<User> list = new ArrayList<User>();
        Cursor c = db.rawQuery(sql, null);
        while (c.moveToNext()) {
            User user = new User(0, null, null);
    ...
    public int updateHongbaoCount(User user){
        String sql = "select hongbao  from hongbao where id = "+user.id;
        int hongbao = 0;
        Cursor c = db.rawQuery(sql, null);
        if (c.moveToNext()) {
